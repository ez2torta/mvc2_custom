...
def build_mini_cvs1j_cdi(output_cdi_path: str, volume_name: str = "CAPCOM_VS_SNK", verbose: bool = True):
    ...
    # 2. Agregar Capcom vs. SNK Japan (Vanilla y Auto-Unlock con Hardlinks)
    cvs_src = os.path.join(GAMES_DIR, "CVS1J")
    stage_game_files(cvs_src, os.path.join(staging_dir, "CVS1J"))

    cvs_unlock_src = os.path.join(GAMES_DIR, "CVS1J_UNLOCK")
    if os.path.exists(cvs_unlock_src):
        stage_game_files(cvs_unlock_src, os.path.join(staging_dir, "CVS1J_UNLOCK"))
