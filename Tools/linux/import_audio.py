...
def matches_audio_filter(adx_key, fname, filters):
    if not filters:
        return True
    key_upper = adx_key.upper()
    fname_upper = fname.upper()
    for f in filters:
        if f in key_upper or key_upper in f or f in fname_upper:
            return True
    return False

def import_all_audio(input_dir, mvc2_dir, filter_str=None):
    ...
    if not os.path.exists(input_dir):
        print(f"[!] Error: No existe el directorio de audio '{input_dir}'.")
        print(f"    Extrae primero los audios con 'make extract-audio' o crea la carpeta '{input_dir}'.")
        return
    ...
    for d in search_dirs:
        ...
        for fname in sorted(os.listdir(d)):
            ...
            if not matches_audio_filter(adx_key, fname, filters):
                continue
...
if __name__ == "__main__":
    ...
    import_all_audio(in_dir, out_dir, filter_arg)
